    ...
    <section className="premium-surface p-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#d9c79f]/80 bg-white px-4 py-2 text-sm font-medium transition hover:bg-[#f5efe5] disabled:cursor-not-allowed disabled:opacity-60 dark:border-[#b8894a]/30 dark:bg-[#0b1728] dark:hover:bg-[#172033]"
        >
      ...
      {references && !hasResults ? (
        <div className="premium-panel mt-5 p-4 text-sm text-slate-600 dark:text-slate-300">
  ...
  return (
    <div className="premium-panel p-4">
      <div className="flex items-center gap-2 font-semibold text-slate-950">
        <Icon className="h-4 w-4 text-[#b8894a]" />
        {title}
      </div>
        ...
        {items.length ? (
          items.map((item) => (
            <Link
              ...
              className="flex items-start justify-between gap-3 rounded-lg border border-[#d9c79f]/50 bg-white p-3 text-sm leading-6 text-slate-700 transition hover:text-[#8a612f] dark:border-[#b8894a]/15 dark:bg-[#07111f] dark:text-slate-200 dark:hover:text-[#f1d79d]"
            >
              <span>{item}</span>
