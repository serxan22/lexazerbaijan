...
const content = sanitizeArticleContent(article.content);
  ...
  return (
    <article className="bg-background dark:bg-slate-950">
      <script
      ...
      <section style={{ position: "relative", zIndex: 0 }} className="premium-hero">
        <div className="legal-container relative z-10 grid gap-10 py-14 lg:grid-cols-[1fr_420px] lg:items-end">
          <div>
            ...
            <h1 className="mt-5 max-w-4xl font-serif text-5xl font-semibold leading-tight text-slate-950 dark:text-white md:text-6xl">
              {article.title}
            </h1>
            {article.subtitle ? <p className="mt-5 max-w-3xl text-xl leading-8 text-slate-600 dark:text-slate-300">{article.subtitle}</p> : null}
            <div className="mt-8 flex flex-wrap items-center gap-5 text-sm text-slate-500">
              <Link href={`/authors/${article.author.username}`} className="flex items-center gap-3 text-slate-800 dark:text-slate-100">
                ...
                <span>
                  <span className="flex items-center gap-2 font-medium">
                    ...
                    {article.author.verifiedWriter ? <VerifiedBadge /> : null}
                  </span>
      ...
      <div className="legal-container relative z-0 grid gap-10 py-12 lg:grid-cols-[1fr_280px]">
        <div className="mx-auto w-full max-w-3xl">
          <div className="rounded-lg border border-gold/25 bg-gold/10 p-5 text-sm leading-6 text-slate-700 dark:text-slate-200">
          ...
          <section className="rounded-lg border bg-card/90 p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <Link href={`/authors/${article.author.username}`} className="mt-1 block font-serif text-2xl font-semibold text-slate-950 dark:text-white">
                  {article.author.fullName}
                </Link>
      ...
      {article.related.length ? (
        <section className="section-shell bg-card">
          <div className="legal-container">
            ...
            <h2 className="mt-3 font-serif text-4xl font-semibold text-slate-950 dark:text-white">{dictionary.article.relatedTitle}</h2>
